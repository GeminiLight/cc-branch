...
>
> For the application-layer refactor history, see
> [`wiki/specs/architecture-application-layer-spec.md`](../wiki/specs/architecture-application-layer-spec.md).
...
- `state_store.py` centralizes load/mutate/save state updates for application
  workflows.
- `session_binding.py` manages best-effort binding of newly created agent
  sessions to local state.
- `agent_sessions.py` discovers reusable conversation sessions from local agent
  CLI stores.
- `ssh_config.py` exposes SSH host discovery for remote target pickers.
- `global_agents.py` manages effective agent profile resolution across layers.
...
- keep starter-config helpers focused on the current public schema
...
- `layoutBackend: tmux` is the persistent managed backend used by
  dashboard/attach/stop/sync
- `layoutBackend: direct` opens visible local terminal/editor windows through
  configured openers and is not treated as a persistent managed session

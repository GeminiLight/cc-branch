...
- 前端 `SlotsSection.tsx` 已抽出更多纯模型逻辑、跨 tab 移动逻辑、agent icon 显示逻辑、canvas rendering 和 session 选择器，但仍然承担较多职责：拖拽事件、inspector 编辑、tmux group 编辑。
...
1. 继续拆分 `SlotsSection.tsx` 的职责；本轮已抽出更多 workspace model、pane movement、canvas rendering 和 session selector，下一步应拆 inspector editing / tmux group editing。

...
不要再把 `slot` / `window` 作为用户可见主术语。它们只允许作为内部实现名、状态文件字段或 tmux 特定诊断细节出现。
...
不要使用 `runtime: terminal` 作为公开配置概念。当前公开 schema 只接受 `layoutBackend: direct | tmux`。
...
| 内部名 | 用户侧名 | 说明 |
|---|---|---|
| `SlotConfig` | Tab | 内部 planner/state 模型，后续可重命名为 `TabConfig`。 |
| `WindowConfig` | Pane | 内部 planner/state 模型，代表可执行 Pane。 |
| `runtime` | layout backend mapping | 内部运行策略字段；公开 schema 用 `layoutBackend`。 |
| `opener` | openWith | opener 是内部适配器实现名；公开配置只写 openWith。 |
...
2. 因为应用尚未正式发布，旧 `slots/windows/runtime/default_opener` 不再作为公开配置读入；遇到这些字段应直接报错。
3. Web UI 表单使用 Tab/Pane 文案；配置编辑模型不保留旧字段占位。
4. State 文件可暂时保留 `slots/windows`，因为它是本机运行元数据，不是公开配置界面。
5. 后续重构再把内部 `SlotConfig` / `WindowConfig` 改名为 `TabConfig` / `PaneConfig`。

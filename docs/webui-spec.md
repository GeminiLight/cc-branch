...
>
> For multi-project dashboard roadmap ideas, see [`wiki/webui-dashboard2-spec.md`](../wiki/webui-dashboard2-spec.md).
> For navigation notes, see [`wiki/webui-sidebar-spec.md`](../wiki/webui-sidebar-spec.md).
...
- `cc-branch service start` in the CLI for the managed local Web UI service
...
- `service start` binds to `127.0.0.1:8080`, returns after launching the app-level local service, and stores service metadata under `~/.cc-branch/app/`
- `serve` binds to `127.0.0.1:8080` in the foreground, uses the selected project scope, and stops with `Ctrl+C`
- serves `index.html`

...
- **让恢复更稳定**：把本地运行信息写进 `.cc-branch.state.toml`，回到项目时更容易接上之前的会话。
...
工作空间结构放在 `.cc-branch.yaml` 里，团队可以共享同一份结构定义，本地运行状态则单独保存在 `.cc-branch.state.toml`。
...
- 查看诊断结果
- 使用内置模板初始化项目
- 在本机系统终端打开总览面板或某个 slot
- 通过 Open With 在 VS Code、Cursor 等编辑器中打开项目目录

...
| tmux unavailable | Config may contain tmux-backed tabs, but this machine cannot run tmux. | Install tmux or switch tabs to direct layout. | Disable tmux lifecycle actions before the user clicks them. |
| Tmux pane missing | Configured tmux pane is not currently running. | Start/update runtime. | Say "not running", not "config mismatch". |
| Tmux pane changed | Running tmux pane uses an older launch spec. | Start/update runtime. | Say "older launch command"; preserve current running state until user applies. |
| Tmux pane untracked | A running pane exists but lacks current metadata. | Start/update runtime. | Explain it was not launched from the current config. |
| Extra tmux pane | A tmux pane exists outside the current config. | Stop extra panes after confirmation. | Separate destructive cleanup from normal sync. |
| Direct layout | External shell/editor process, not managed by CC Branch. | Open manually through an opener. | Do not offer background start/stop semantics. |

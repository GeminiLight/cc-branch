...
class OpenerTests(unittest.TestCase):
    ...
    def test_list_openers_reports_editor_capabilities(self):
        ...
        with (
            ...
            patch("cc_branch.openers.registry._extra_command_dirs", return_value=[]),
            patch("cc_branch.openers.registry._macos_app_cli_candidates", return_value=[]),
            patch("cc_branch.openers.registry._login_shell_command_path", return_value=None),
        ):
    ...
    def test_editor_cli_detection_searches_common_paths_when_desktop_path_is_minimal(self):
        """Desktop-launched macOS apps should find editor CLIs outside launchd's PATH."""
        with TemporaryDirectory() as tmp:
            bin_dir = Path(tmp) / "bin"
            bin_dir.mkdir()
            code_cli = bin_dir / "code"
            code_cli.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
            os.chmod(code_cli, 0o755)

            with (
                patch("cc_branch.openers.registry.shutil.which", return_value=None),
                patch("cc_branch.openers.registry._extra_command_dirs", return_value=[bin_dir]),
                patch("cc_branch.openers.registry._macos_app_cli_candidates", return_value=[]),
                patch("cc_branch.openers.registry._login_shell_command_path", return_value=None),
            ):
                payload = list_openers()
        ...
        openers = {opener["id"]: opener for opener in payload["openers"]}
        self.assertTrue(openers["vscode"]["available"])
        self.assertEqual(openers["vscode"]["executable"], str(code_cli))
        self.assertFalse(openers["cursor"]["available"])

    def test_editor_cli_detection_uses_login_shell_when_desktop_path_is_minimal(self):
        """If zsh can find an editor CLI, desktop opener detection should use the same fallback."""
        with TemporaryDirectory() as tmp:
            code_cli = Path(tmp) / "code"
            code_cli.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
            os.chmod(code_cli, 0o755)
            ...
            with (
                ...
                patch("cc_branch.openers.registry._extra_command_dirs", return_value=[]),
                patch("cc_branch.openers.registry._macos_app_cli_candidates", return_value=[]),
                patch("cc_branch.openers.registry._login_shell_command_path", side_effect=lambda command: code_cli if command == "code" else None),
            ):
                payload = list_openers()
    ...
    def test_login_shell_command_path_accepts_absolute_executable_output(self):
        """The login-shell fallback should parse an absolute path from command -v output."""
        from cc_branch.openers import registry

        with TemporaryDirectory() as tmp:
            root = Path(tmp)
            code_cli = root / "code"
            shell = root / "fake-shell"
            code_cli.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
            shell.write_text(f"#!/bin/sh\nprintf '%s\\n' {str(code_cli)!r}\n", encoding="utf-8")
            os.chmod(code_cli, 0o755)
            os.chmod(shell, 0o755)

            registry._login_shell_command_path.cache_clear()
            with patch("cc_branch.openers.registry._candidate_login_shells", return_value=[str(shell)]):
                self.assertEqual(registry._login_shell_command_path("code"), code_cli)
            registry._login_shell_command_path.cache_clear()

    def test_macos_editor_cli_detection_falls_back_to_app_bundle_cli(self):
        """VS Code and Cursor app bundles include a CLI that can open folders."""
        with TemporaryDirectory() as tmp:
            code_cli = Path(tmp) / "Visual Studio Code.app" / "Contents" / "Resources" / "app" / "bin" / "code"
            ...
            with (
                ...
                patch("cc_branch.openers.registry.shutil.which", return_value=None),
                patch("cc_branch.openers.registry._extra_command_dirs", return_value=[]),
                patch("cc_branch.openers.registry._macos_app_cli_candidates", return_value=[code_cli]),
            ):
                payload = list_openers()
    ...
    def test_macos_cursor_detection_accepts_code_bin_inside_cursor_bundle(self):
        """Some Cursor installs expose the bundle CLI as bin/code instead of bin/cursor."""
        with TemporaryDirectory() as tmp:
            cursor_app = Path(tmp) / "Cursor.app"
            cursor_cli = cursor_app / "Contents" / "Resources" / "app" / "bin" / "code"
            cursor_cli.parent.mkdir(parents=True)
            cursor_cli.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
            os.chmod(cursor_cli, 0o755)

            def fake_find_macos_app(app_name: str) -> Path | None:
                return cursor_app if app_name == "Cursor" else None

            with (
                patch("cc_branch.openers.registry.sys.platform", "darwin"),
                patch("cc_branch.openers.registry.shutil.which", return_value=None),
                patch("cc_branch.openers.registry._extra_command_dirs", return_value=[]),
                patch("cc_branch.openers.registry._find_macos_app", side_effect=fake_find_macos_app),
                patch("cc_branch.openers.registry._login_shell_command_path", return_value=None),
            ):
                payload = list_openers()

        openers = {opener["id"]: opener for opener in payload["openers"]}
        self.assertTrue(openers["cursor"]["available"])
        self.assertEqual(openers["cursor"]["executable"], str(cursor_cli))

    def test_windows_command_detection_checks_cmd_wrappers(self):
        """Windows editor installers usually expose code.cmd or cursor.cmd."""
        from cc_branch.openers.registry import _command_names

        with patch("cc_branch.openers.registry.os.name", "nt"):
            self.assertEqual(_command_names("code"), ["code", "code.cmd", "code.exe", "code.bat"])
    ...
    def test_global_openers_are_available_without_project_config(self):
        """User-level openers should be listed even before a workspace has local opener config."""
        from cc_branch.application.config_workflows.options import opener_options
        from cc_branch.models import OpenerSpec

        with (
            patch("cc_branch.application.config_workflows.options.load_global_openers", return_value={
                "ghostty": OpenerSpec(label="Ghostty", kind="terminal", command="ghostty"),
            }),
            patch("cc_branch.openers.registry.shutil.which", return_value="/opt/homebrew/bin/ghostty"),
        ):
            result = opener_options(Path("/tmp/missing/.cc-branch/config.yaml"))

        openers = {opener["id"]: opener for opener in result.payload["openers"]}
        self.assertEqual(openers["ghostty"]["label"], "Ghostty")
        self.assertTrue(openers["ghostty"]["available"])
...
"""
    ...
    def test_warp_layout_uses_stable_project_name_and_removes_stale_hash_configs(self):

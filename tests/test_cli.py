...
class CLITests(unittest.TestCase):
    ...
    def test_package_exposes_release_version(self):
        ...
        self.assertEqual(cc_branch.__version__, match.group(1))

    def test_parser_version_uses_local_package_constant(self):
        from cc_branch.cli.parser import _package_version

        self.assertEqual(_package_version(), cc_branch.__version__)
    ...
    def test_status_accepts_format_after_subcommand(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write_workspace(root)
            stdout = StringIO()
            with redirect_stdout(stdout):
                exit_code = main(["--project", str(root), "status", "--format", "json"])
    ...
    def test_service_start_spawns_detached_webui_server(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            home = root / "home"
            ...
            with (
                ...
                patch("cc_branch.cli.commands.service.process_is_running", return_value=False),
                patch("cc_branch.webui.server.static.static_assets_available", return_value=True),
                patch("cc_branch.cli.commands.service.subprocess.Popen", return_value=process) as popen,
                redirect_stdout(stdout),
            ):
                exit_code = main(["service", "start"])

            self.assertEqual(exit_code, 0)
            metadata_path = home / ".cc-branch/app/webui-service.json"
            metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
            self.assertEqual(metadata["pid"], 12345)
            self.assertEqual(metadata["url"], "http://127.0.0.1:8080")
            self.assertEqual(
                metadata["config_path"],
                str(home / ".cc-branch/app/backend-workspace/.cc-branch/config.yaml"),
            )
            self.assertFalse((root / ".cc-branch").exists())
            command = popen.call_args.args[0]
            self.assertEqual(command[:3], [sys.executable, "-m", "cc_branch"])
            self.assertIn("--project", command)
            self.assertIn(str(home / ".cc-branch/app/backend-workspace"), command)
            self.assertIn("serve", command)
            self.assertIn("Started CC Branch Web UI service", stdout.getvalue())

    def test_service_start_reuses_running_webui_server(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            home = root / "home"
            metadata_path = home / ".cc-branch/app/webui-service.json"
            ...
            metadata_path.write_text(
                json.dumps({"pid": 12345, "url": "http://127.0.0.1:8080"}),
                encoding="utf-8",
            )
            ...
            with (
                patch("cc_branch.cli.Path.cwd", return_value=root),
                patch("cc_branch.app_state.paths.Path.home", return_value=home),
                patch("cc_branch.cli.commands.service.process_is_running", return_value=True),
                patch("cc_branch.cli.commands.service.subprocess.Popen") as popen,
                redirect_stdout(stdout),
            ):
                exit_code = main(["service", "start"])
    ...
    def test_service_stop_terminates_recorded_webui_server(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            home = root / "home"
            metadata_path = home / ".cc-branch/app/webui-service.json"
            ...
            with (
                patch("cc_branch.cli.Path.cwd", return_value=root),
                patch("cc_branch.app_state.paths.Path.home", return_value=home),
                patch("cc_branch.cli.commands.service.process_is_running", side_effect=[True, False]),
                patch("cc_branch.cli.commands.service.terminate_process") as terminate,
                redirect_stdout(stdout),
            ):
            ...
            self.assertIn("Stopped CC Branch Web UI service", stdout.getvalue())
    ...
    def test_help_targets_explains_public_target_syntax(self):
        ...
        rendered = stdout.getvalue()
        self.assertIn("dev:planner", rendered)
        self.assertIn("target", rendered.lower())

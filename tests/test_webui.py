...
class TerminalOpenTests(unittest.TestCase):
    ...
    def test_open_terminal_preserves_quoted_cli_path_for_dashboard(self):
        """Terminal opener should not split executable paths on spaces."""
        from unittest.mock import patch
    ...
    def test_open_terminal_parses_quoted_attach_target(self):
        """Terminal opener should pass a raw target, not a pre-quoted shell fragment."""
        from unittest.mock import patch
...
class StaticAssetTests(unittest.TestCase):
    ...
    def test_missing_static_assets_message_names_recovery_paths(self):
        ...
        self.assertIn("pip install --upgrade --force-reinstall cc-branch", message)
        self.assertIn("GitHub Releases", message)
...
"""
    ...
    def test_api_status_returns_json(self):
        ...
        try:
            ...
            with (
                ...
                urlopen(f"http://127.0.0.1:{port}/api/status", timeout=HTTP_TIMEOUT) as response,
            ):
    ...
    def test_api_info_reports_actual_bound_port(self):
        ...
        try:
            with (
                patch("cc_branch.webui.server.api.default_shell_command", return_value="zsh"),
                patch("cc_branch.webui.server.api.discover_ssh_hosts", return_value=[{"alias": "devbox"}]),
                urlopen(f"http://127.0.0.1:{port}/api/info", timeout=HTTP_TIMEOUT) as response,
            ):
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
            ...
            self.assertEqual(data["default_shell"], "zsh")
            self.assertEqual(data["ssh_hosts"], [{"alias": "devbox"}])
        finally:
            self._stop_test_server(server)

    def test_api_info_reports_desktop_backend_source(self):
        """Packaged-app smoke tests should know whether the bundled sidecar answered."""
        from unittest.mock import patch

        server, port = self._start_test_server()
        try:
            with (
                patch.dict("os.environ", {"CC_BRANCH_BACKEND_SOURCE": "bundled-sidecar"}),
                urlopen(f"http://127.0.0.1:{port}/api/info", timeout=HTTP_TIMEOUT) as response,
            ):
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())

            self.assertEqual(data["backend_source"], "bundled-sidecar")
        finally:
            self._stop_test_server(server)

    def test_api_info_reports_desktop_metadata_from_environment(self):
        """Packaged desktop smoke tests should identify the exact launched asset."""
        from unittest.mock import patch
        ...
        try:
            with (
                patch.dict(
                    "os.environ",
                    {
                        "CC_BRANCH_DESKTOP_VERSION": "1.0.2",
                        "CC_BRANCH_DESKTOP_PLATFORM": "darwin",
                        "CC_BRANCH_DESKTOP_ARCH": "aarch64",
                    },
                ),
                urlopen(f"http://127.0.0.1:{port}/api/info", timeout=HTTP_TIMEOUT) as response,
            ):
            ...
            self.assertEqual(data["desktop_version"], "1.0.2")
            self.assertEqual(data["desktop_platform"], "darwin")
            self.assertEqual(data["desktop_arch"], "aarch64")
        finally:
            self._stop_test_server(server)
...
""")
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/api/status", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
    ...
    def test_tauri_desktop_origin_is_allowed_for_api(self):
        """Packaged desktop windows can call the local sidecar API."""
        server, port = self._start_test_server()
        try:
    ...
    def test_tauri_desktop_origin_can_mutate_local_sidecar_api(self):
        """Packaged desktop windows can write through the local sidecar API."""
        from urllib.error import HTTPError

        server, port = self._start_test_server()
        try:
            request = Request(
                f"http://127.0.0.1:{port}/api/projects/config",
                data=b"{}",
                headers={
                    "Content-Type": "application/json",
    ...
    def test_api_status_rejects_config_path_outside_project_metadata(self):
        ...
        outside_config.write_text("version: 2\nproject: outside\nroot: .\ntabs: []\n", encoding="utf-8")
    ...
    def test_api_configs_lists_selectable_project_configs(self):
        ...
        review_config = self.cwd / ".cc-branch/configs/review.yaml"
        review_config.parent.mkdir(parents=True, exist_ok=True)
        review_config.write_text("version: 2\nproject: review\nroot: .\ntabs: []\n", encoding="utf-8")

        server, port = self._start_test_server()
        try:
            project_path = quote(str(self.cwd))
            with urlopen(f"http://127.0.0.1:{port}/api/configs?project_path={project_path}", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
    ...
    def test_api_configs_create_rename_and_delete_named_configs(self):
        ...
        try:
            ...
            with urlopen(rename, timeout=HTTP_TIMEOUT) as response:
            ...
            with urlopen(delete, timeout=HTTP_TIMEOUT) as response:
    ...
    def test_api_openers_returns_registered_openers(self):
        ...
        try:
            with patch("cc_branch.openers.registry.shutil.which", return_value="/usr/local/bin/code"):
                with urlopen(f"http://127.0.0.1:{port}/api/openers", timeout=HTTP_TIMEOUT) as response:
                    self.assertEqual(response.status, 200)
                    data = json.loads(response.read().decode())
    ...
    def test_api_config_returns_content(self):
        ...
        try:
            with (
                patch("cc_branch.runtime.backends.TmuxBackend.available", return_value=False),
                urlopen(f"http://127.0.0.1:{port}/api/config", timeout=HTTP_TIMEOUT) as response,
            ):
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
    ...
    def test_api_config_returns_json_for_invalid_yaml(self):
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/api/config", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
    ...
    def test_api_doctor_returns_report(self):
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/api/doctor", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
...
""")
    ...
    def test_api_project_probe_without_query_uses_project_root(self):
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/api/project/probe", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
    ...
    def test_api_remote_list_directories_returns_ssh_listing(self):
        """Add Project SSH flow should be able to browse remote directories."""
        from unittest.mock import patch

        server, port = self._start_test_server()
        try:
            with patch(
                "cc_branch.webui.server.api.list_remote_directories",
                return_value={
                    "path": "/srv",
                    "parent": "/",
                    "entries": [{"name": "app", "path": "/srv/app"}],
                },
            ) as list_remote_directories:
                request = Request(
                    f"http://127.0.0.1:{port}/api/remote/list-directories",
                    data=json.dumps({
            ...
            self.assertEqual(data["entries"], [{"name": "app", "path": "/srv/app"}])
            list_remote_directories.assert_called_once_with(
    ...
    def test_api_global_agents_reads_and_saves_user_override(self):
        ...
        try:
            with patch("cc_branch.agent_registry.paths.Path.home", return_value=home_dir):
                with urlopen(f"http://127.0.0.1:{port}/api/agents/global", timeout=HTTP_TIMEOUT) as response:
                    data = json.loads(response.read().decode())
    ...
    def test_api_global_openers_reads_and_saves_user_override(self):
        """Settings should be able to manage the user-level openers file."""
        from unittest.mock import patch

        home_dir = self.cwd / "home-openers"
        home_dir.mkdir()
        server, port = self._start_test_server()
        try:
            with patch("cc_branch.opener_registry.Path.home", return_value=home_dir):
                with urlopen(f"http://127.0.0.1:{port}/api/openers/global", timeout=HTTP_TIMEOUT) as response:
                    data = json.loads(response.read().decode())
                self.assertFalse(data["exists"])
                self.assertEqual(data["path"], str(home_dir / ".cc-branch/openers.yaml"))
                self.assertIn("openers: {}", data["content"])

                request = Request(
                    f"http://127.0.0.1:{port}/api/openers/global",
                    data=json.dumps({
                        "content": "openers:\n  ghostty:\n    label: Ghostty\n    command: ghostty\n",
                        "base_mtime": data["mtime"],
                        "base_content_hash": data["content_hash"],
                    }).encode(),
                    headers={"Content-Type": "application/json"},
                    method="POST",
                )
                ...
                self.assertTrue(saved["success"])
                self.assertTrue(saved["exists"])
                self.assertIn("ghostty", {opener["id"] for opener in saved["user_openers"]})
                self.assertTrue((home_dir / ".cc-branch/openers.yaml").exists())
        finally:
            self._stop_test_server(server)
    ...
    def test_api_projects_endpoints_manage_global_index(self):
        ...
        try:
            with patch("cc_branch.app_state.paths.Path.home", return_value=home_dir):
                with urlopen(f"http://127.0.0.1:{port}/api/projects", timeout=HTTP_TIMEOUT) as response:
                    data = json.loads(response.read().decode())
                ...
                with urlopen(add_request, timeout=HTTP_TIMEOUT) as response:
                ...
                with urlopen(activate_request, timeout=HTTP_TIMEOUT) as response:
                ...
                with urlopen(pin_request, timeout=HTTP_TIMEOUT) as response:
                ...
                with urlopen(add_alt_request, timeout=HTTP_TIMEOUT) as response:
                ...
                with urlopen(reorder_request, timeout=HTTP_TIMEOUT) as response:
                ...
                with urlopen(remove_request, timeout=HTTP_TIMEOUT) as response:
                    removed = json.loads(response.read().decode())
                self.assertEqual([project["id"] for project in removed["projects"]], [alt_project_id])
        finally:
            self._stop_test_server(server)

    def test_api_projects_current_does_not_inject_app_scoped_backend_workspace(self):
        """App-scoped service backends should not appear as a current project."""
        from unittest.mock import patch
        ...
        try:
            with (
                ...
                patch.dict(os.environ, {"CC_BRANCH_BACKEND_SCOPE": "app"}),
            ):
                request = Request(
                    f"http://127.0.0.1:{port}/api/projects/current",
                    data=b"",
    ...
    def test_api_projects_add_accepts_remote_project(self):
        """Adding a project can create a local metadata workspace for an SSH directory."""
        from unittest.mock import patch
        ...
        try:
            ...
            project = added["projects"][0]
            self.assertEqual(project["name"], "remote-app")
            self.assertEqual(project["display_path"], "ssh://ubuntu@gpu-dev/srv/app")
            self.assertEqual(project["remote"]["host"], "gpu-dev")
...
""")
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/api/agents", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
                data = json.loads(response.read().decode())
    ...
    def test_api_agent_sessions_returns_local_options(self):
        ...
        try:
            ...
            with patch("cc_branch.webui.server.api.agent_session_options", return_value=result) as options:
                with urlopen(f"http://127.0.0.1:{port}/api/agent-sessions?agent=codex", timeout=HTTP_TIMEOUT) as response:
                    self.assertEqual(response.status, 200)
                    data = json.loads(response.read().decode())
    ...
    def test_api_agent_sessions_uses_selected_config_scope(self):
        ...
        try:
            ...
            with patch("cc_branch.webui.server.api.agent_session_options", return_value=result) as options:
                with urlopen(f"http://127.0.0.1:{port}/api/agent-sessions?{query}", timeout=HTTP_TIMEOUT) as response:
                    self.assertEqual(response.status, 200)
...
""")
    ...
    def test_root_returns_html(self):
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
    ...
    def test_static_binary_file_served_without_text_decoding(self):
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/favicon.png", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
    ...
    def test_hashed_static_assets_are_immutable(self):
        ...
        server, port = self._start_test_server()
        try:
            with urlopen(f"http://127.0.0.1:{port}/assets/{asset_name}", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
                self.assertEqual(
    ...
    def test_root_requires_auth_when_token_enabled(self):
        ...
        try:
            with self.assertRaises(HTTPError) as ctx:
                urlopen(f"http://127.0.0.1:{port}/", timeout=HTTP_TIMEOUT)
    ...
    def test_root_token_query_seeds_auth_cookie(self):
        ...
        try:
            ...
            with opener.open(f"http://127.0.0.1:{port}/?token=secret-token", timeout=HTTP_TIMEOUT) as response:
                self.assertEqual(response.status, 200)
    ...
    def test_get_api_requires_auth_when_token_enabled(self):
        ...
        try:
            with self.assertRaises(HTTPError) as ctx:
                urlopen(f"http://127.0.0.1:{port}/api/config", timeout=HTTP_TIMEOUT)
            self.assertEqual(ctx.exception.code, 401)
        finally:
            self._stop_test_server(server)
    ...
    def test_404_for_unknown_paths(self):
        ...
        try:
            with self.assertRaises(HTTPError):
                urlopen(f"http://127.0.0.1:{port}/unknown", timeout=HTTP_TIMEOUT)
        finally:
            self._stop_test_server(server)
...
class PackagingMetadataTests(unittest.TestCase):
    ...
    def test_python_publish_workflow_builds_webui_before_package(self):
        """PyPI wheels should include prebuilt Web UI assets, not build on install."""
        root = Path(__file__).resolve().parents[1]
        workflow = (root / ".github/workflows/publish-python.yml").read_text(encoding="utf-8")

        self.assertIn("npm ci", workflow)
        self.assertIn("npm run test", workflow)
        self.assertIn("python scripts/build-webui.py", workflow)
        self.assertLess(
            workflow.index("python scripts/build-webui.py"),

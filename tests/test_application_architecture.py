...
class ConfigWorkflowTests(unittest.TestCase):
    ...
    def test_collect_config_issues_warns_for_unknown_fields(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(issues[1].target, "tab:dev")
    ...
    def test_collect_config_issues_errors_for_invalid_enums(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(errors[1].target, "tab:dev")
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertCountEqual(
                ...
                [
                    ("agent:codex", "command"),
                    ("tab:tab[0]", "name"),
                    ("pane:tab[0]:pane[0]", "name"),
                    ("pane:tab[0]:pane[0]", "agent"),
                ],
            )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertCountEqual(
                ...
                [
                    ...
                    ("missing_launch_command", "pane:dev:main"),
                ],
            )
    ...
    def test_collect_config_issues_validates_remote_fields(self):
        from cc_branch.application.config_validation import collect_config_issues
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            errors = [
                (issue.target, issue.context["field"], issue.context["expected"])
                for issue in issues
                if issue.issue_type == "invalid_type"
            ]
            self.assertIn(("tab:dev.remote", "port", "1-65535"), errors)
            self.assertIn(("pane:dev:worker.remote", "args", "list of strings"), errors)
            self.assertNotIn(("pane:dev:logs.remote", "host", "non-empty string"), errors)
            self.assertNotIn(("pane:dev:local", "remote", "string or mapping"), errors)
    ...
    def test_collect_config_issues_errors_for_invalid_env_keys(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertCountEqual(
                ...
                [("tab:dev", "BAD-KEY"), ("pane:dev:main", "ALSO.BAD")],
            )
    ...
    def test_collect_config_issues_errors_for_invalid_container_shapes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(("panes", "list"), shape_errors)
...
class WorkspaceActionsTests(unittest.TestCase):
    ...
    def test_terminal_open_target_keeps_selected_named_config_in_attach_command(self):
        from unittest.mock import patch
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                patch(
                    ...
                    side_effect=lambda _opener, cap, _custom=None: cap == "attach_target",
                ),
                patch(
                    "cc_branch.application.workspace_actions.opener_label",
                    return_value="System Terminal",
                ),
                patch("cc_branch.application.workspace_actions.ensure_slot", return_value=[]) as ensure_slot,
                patch("cc_branch.application.workspace_actions.open_with") as open_with,
            ):
            ...
            ensure_slot.assert_called_once()
            self.assertEqual(
                open_with.call_args.kwargs["cli"],
                f"cc-branch --config {config_path.resolve()}",
            )
            self.assertEqual(open_with.call_args.kwargs["intent"].kind, "attach_target")
            self.assertEqual(open_with.call_args.kwargs["intent"].target, "review")

    def test_terminal_open_dashboard_keeps_selected_named_config_in_attach_command(self):
        from unittest.mock import patch
    ...
    def test_attach_workspace_accepts_public_tab_target(self):
        from unittest.mock import patch
    ...
    def test_execute_workspace_action_accepts_public_tab_targets(self):
        from unittest.mock import patch

...
class PlannerTests(unittest.TestCase):
    ...
    def test_plan_workspace_wraps_remote_commands_with_ssh(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            window = plan.slots[0].windows[0]
            self.assertEqual(window.command_binary, "ssh")
            self.assertEqual(window.remote.host, "gpu-dev")
            self.assertEqual(
                window.launch_command,
                "ssh -p 2222 -o ServerAliveInterval=30 -A ubuntu@gpu-dev "
                "'cd /srv/app && env CUDA_VISIBLE_DEVICES=0 python train.py'",
            )

    def test_plan_workspace_merges_remote_overrides(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write(
                root / ".cc-branch/config.yaml",
            ...
            window = plan.slots[0].windows[0]
            self.assertEqual(window.remote.target(), "app@devbox")
            self.assertEqual(window.remote.cwd, "/var/log/app")
            self.assertEqual(window.launch_command, "ssh app@devbox 'cd /var/log/app && tail -f app.log'")

    def test_plan_workspace_allows_local_override_under_remote_tab(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write(
                root / ".cc-branch/config.yaml",
            ...
            self.assertEqual(plan.slots[0].windows[0].launch_command, "ssh devbox 'uname -a'")
            self.assertIsNone(plan.slots[0].windows[1].remote)
            self.assertEqual(plan.slots[0].windows[1].launch_command, "echo local")

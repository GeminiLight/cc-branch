...
class ProjectIndexStoreTests(unittest.TestCase):
    ...
    def test_add_remote_project_creates_local_workspace_with_remote_metadata(self):
        ...
        self.assertEqual(project["remote"], {"host": "gpu-dev", "user": "ubuntu", "port": 2222, "cwd": "/srv/app"})
        self.assertTrue(Path(str(project["path"])).exists())
        config_path = Path(str(project["path"])) / ".cc-branch" / "config.yaml"

...
class WorkspacePlannerTests(unittest.TestCase):
    """Workspace integration tests plus a few legacy TOML compatibility checks."""
    ...
    def test_plan_bootstraps_generated_session_id_and_label(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            workspace = load_workspace(root / ".cc-branch.toml")
    ...
    def test_apply_workspace_creates_tmux_sessions_for_tmux_and_shell_slots(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("demo-scratch", sessions)
            finally:
                subprocess.run(["tmux", "kill-session", "-t", "demo-dev"], check=False)
                subprocess.run(["tmux", "kill-session", "-t", "demo-scratch"], check=False)

    def test_cc_branch_shell_wrapper_initializes_project(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            result = subprocess.run(
                [str(self.repo_root / "bin" / "cc-branch"), "init", "--bootstrap-sessions"],
                cwd=root,
                capture_output=True,
                text=True,
                check=True,
    ...
    def test_plan_applies_env_to_launch_command(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self._write(
                ...
                name = "scratch"
                backend = "shell"
                env = { APP_MODE = "local dev", PORT = 8123 }
    ...
    def test_stop_workspace_accepts_window_target(self) -> None:
        ...
        with (
            ...
            patch("cc_branch.runtime._kill_dashboard") as kill_dashboard_mock,
            patch("cc_branch.runtime.tmux_has_window", return_value=True),
    ...
    def test_restart_workspace_recreates_slot_detached(self) -> None:
        ...
        with (
            patch("cc_branch.runtime.which", return_value="/usr/bin/tmux"),
            patch("cc_branch.runtime._kill_dashboard") as kill_dashboard_mock,
            patch("cc_branch.runtime.tmux_has_session", return_value=True),
            patch("cc_branch.runtime.ensure_slot") as ensure_slot_mock,
            patch("cc_branch.runtime.attach_slot") as attach_slot_mock,
            patch("cc_branch.runtime.subprocess.run") as run_mock,
        ):
        ...
        ensure_slot_mock.assert_called_once_with(plan.slots[0])
    ...
    def test_open_dashboard_applies_layout_from_display_config(self) -> None:
        ...
        with (
            patch("cc_branch.runtime.which", return_value="/usr/bin/tmux"),
            patch("cc_branch.runtime.apply_workspace"),
    ...
    def test_open_dashboard_uses_shell_wrapper_helper(self) -> None:
        ...
        with (
            ...
            patch("cc_branch.runtime.tmux_has_session", return_value=False),
            patch(
                "cc_branch.runtime.tmux_attach_shell_command",

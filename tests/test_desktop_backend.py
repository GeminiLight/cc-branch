from __future__ import annotations

import unittest
from pathlib import Path
from unittest.mock import patch

from cc_branch import desktop_backend


class DesktopBackendTests(unittest.TestCase):
    def test_parser_requires_backend_paths_and_port(self):
        args = desktop_backend.build_parser().parse_args([
            "--port",
            "8765",
            "--config",
            "/tmp/project/.cc-branch/config.yaml",
            "--state",
            "/tmp/project/.cc-branch/state.yaml",
        ])

        self.assertEqual(args.host, "127.0.0.1")
        self.assertEqual(args.port, 8765)
        self.assertEqual(args.config, "/tmp/project/.cc-branch/config.yaml")
        self.assertEqual(args.state, "/tmp/project/.cc-branch/state.yaml")
        self.assertIsNone(args.token)

    def test_parser_does_not_inherit_web_token_from_user_environment(self):
        with patch.dict("os.environ", {"CC_BRANCH_WEB_TOKEN": "user-shell-token"}):
            args = desktop_backend.build_parser().parse_args([
                "--port",
                "8765",
                "--config",
                "/tmp/project/.cc-branch/config.yaml",
                "--state",
                "/tmp/project/.cc-branch/state.yaml",
            ])

        self.assertIsNone(args.token)

    def test_parser_allows_explicit_token_for_diagnostics(self):
        with patch.dict("os.environ", {"CC_BRANCH_WEB_TOKEN": "user-shell-token"}):
            args = desktop_backend.build_parser().parse_args([
                "--port",
                "8765",
                "--config",
                "/tmp/project/.cc-branch/config.yaml",
                "--state",
                "/tmp/project/.cc-branch/state.yaml",
                "--token",
                "explicit-token",
            ])

        self.assertEqual(args.token, "explicit-token")

    def test_main_starts_webui_server_with_explicit_paths(self):
        with patch("cc_branch.desktop_backend.start_server") as start_server:
            result = desktop_backend.main([
                "--host",
                "127.0.0.1",
                "--port",
                "8765",
                "--config",
                "/tmp/project/.cc-branch/config.yaml",
                "--state",
                "/tmp/project/.cc-branch/state.yaml",
                "--token",
                "secret",
            ])

        self.assertEqual(result, 0)
        start_server.assert_called_once_with(
            Path("/tmp/project/.cc-branch/config.yaml"),
            Path("/tmp/project/.cc-branch/state.yaml"),
            host="127.0.0.1",
            port=8765,
            token="secret",
        )


if __name__ == "__main__":
    unittest.main()

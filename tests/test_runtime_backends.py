...
class TmuxBackendTargetTests(unittest.TestCase):
    def test_session_lookup_uses_exact_target(self):
        backend = TmuxBackend()
        with (
            patch("cc_branch.runtime.backends.which", return_value="tmux"),
            patch("cc_branch.runtime.backends.subprocess.run") as run,
        ):
    ...
    def test_tmux_commands_use_resolved_executable_path(self):
        backend = TmuxBackend()
        with (
            patch("cc_branch.runtime.backends.which", return_value="/opt/homebrew/bin/tmux"),
            patch("cc_branch.runtime.backends.subprocess.run") as run,
        ):
            run.return_value.returncode = 0

            self.assertTrue(backend.available())
            self.assertTrue(backend.has_session("dev"))

        self.assertEqual(
            run.call_args.args[0],
            ["/opt/homebrew/bin/tmux", "has-session", "-t", "=dev"],
        )

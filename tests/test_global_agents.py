...
from cc_branch.agent_registry import list_builtin_agents
...
class GlobalAgentsTests(unittest.TestCase):
    ...
    def test_builtin_registry_replaces_aider_with_copilot_and_opencode(self):
        agents = {agent.name: agent for agent in list_builtin_agents()}

        self.assertEqual(agents["copilot"].command, "copilot")
        self.assertEqual(agents["opencode"].command, "opencode")
        self.assertNotIn("aider", agents)

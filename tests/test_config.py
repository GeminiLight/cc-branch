...
from cc_branch.application.config_workflows import set_window_enabled
from cc_branch.application.workspace_actions.command_specs import command_specs
...
class ConfigTests(unittest.TestCase):
    ...
    def test_window_enabled_state_round_trips_and_stays_out_of_defaults(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
            ...
            workspace = load_workspace(config_path)
            self.assertTrue(workspace.slots[0].windows[0].enabled)
            self.assertFalse(workspace.slots[0].windows[1].enabled)

            public_panes = workspace.to_dict()["tabs"][0]["panes"]
            self.assertNotIn("enabled", public_panes[0])
            self.assertIs(public_panes[1]["enabled"], False)

    def test_plan_keeps_disabled_windows_but_marks_them_not_launchable(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
            ...
            workspace = load_workspace(config_path)
            plan = plan_workspace(workspace, load_state(root / ".cc-branch/state.yaml"), False)

            self.assertEqual([window.name for window in plan.slots[0].windows], ["main", "docs"])
            self.assertEqual([window.name for window in plan.slots[0].launchable_windows()], ["main"])

    def test_terminal_command_specs_skip_disabled_windows(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
            ...
            workspace = load_workspace(config_path)
            plan = plan_workspace(workspace, load_state(root / ".cc-branch/state.yaml"), False)
            specs = command_specs.terminal_command_specs(plan.slots)

            self.assertEqual([spec.title for spec in specs], ["dev:main"])

    def test_set_window_enabled_updates_saved_config(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            ...
            result = set_window_enabled(config_path, state_path, "dev:docs", False)

            self.assertTrue(result.ok)
            workspace = load_workspace(config_path)
            self.assertFalse(workspace.slots[0].windows[1].enabled)
            self.assertIs(workspace.to_dict()["tabs"][0]["panes"][1]["enabled"], False)
    ...
    def test_load_workspace_parses_tabs(self):
        """Test that load_workspace correctly parses public tab definitions."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
    ...
    def test_load_workspace_parses_canonical_workspace_terms(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(workspace.open_with, "cursor")
    ...
    def test_load_workspace_parses_and_serializes_remote_targets(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
            ...
            self.assertIsNotNone(slot.remote)
            self.assertEqual(slot.remote.host, "gpu-dev")
            self.assertEqual(slot.remote.user, "ubuntu")
            self.assertEqual(slot.remote.port, 2222)
            self.assertEqual(slot.remote.cwd, "/srv/app")
            self.assertEqual(slot.remote.args, ["-A"])
            self.assertEqual(slot.remote.options, {"ServerAliveInterval": 30})
            self.assertIsNone(slot.windows[0].remote)
            self.assertEqual(slot.windows[1].remote.cwd, "/var/log/app")
            self.assertFalse(slot.windows[2].remote.enabled)

            public = workspace.to_dict()["tabs"][0]
            self.assertEqual(public["remote"]["host"], "gpu-dev")
            self.assertEqual(public["remote"]["args"], ["-A"])
            self.assertEqual(public["panes"][1]["remote"]["cwd"], "/var/log/app")
            self.assertNotIn("host", public["panes"][1]["remote"])
            self.assertIs(public["panes"][2]["remote"], False)
    ...
    def test_workspace_to_dict_serializes_canonical_terms(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn("open_with", serialized)
    ...
    def test_workspace_rejects_old_slots_field(self):
        """Public configs should not accept pre-release slots format."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
    ...
    def test_workspace_rejects_old_pane_runtime_field(self):
        """Public configs should use layoutBackend instead of runtime."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
    ...
    def test_load_workspace_preserves_open_with_ids(self):
        """Public openWith should load registered opener ids directly."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
            ...
            self.assertEqual(workspace.open_with, "terminal-app")
            self.assertEqual(workspace.to_dict()["openWith"], "terminal-app")

    def test_load_workspace_rejects_old_session_id_field(self):
        """Public config should use session, not session_id."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            config_path = root / ".cc-branch/config.yaml"
            self._write(
    ...
    def test_config_options_list_default_and_named_configs(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self._write(root / ".cc-branch/config.yaml", "version: 2\nproject: default\nroot: .\ntabs: []")
            self._write(root / ".cc-branch/configs/review.yaml", "version: 2\nproject: review\nroot: .\ntabs: []")

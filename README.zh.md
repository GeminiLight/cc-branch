...
> 用一个项目配置启动和恢复多个 Agent CLI 与 shell 窗格。

CC Branch（`cc-branch`，短别名 `ccb`）是面向 AI 编程工作流的本地工作空间管理工具。它记录工作区标签页、窗格、Agent 命令、工作目录和本机 session 元数据，让同一套工作环境可以快速重新打开。

## 使用场景

- 将 planner、coder、reviewer、dev server 和 shell 窗格组成一套可重复打开的项目工作台。
- 跨天继续同一个 AI 编程现场，不需要手动重新搭建窗格。
- 团队共享项目工作台模板，同时保留各自本机的 session 状态。

## 功能特色

- **工作台模板**：覆盖 solo、pair、minimal 等常见 AI 编程工作流。
- **启动与连接命令**：预览、启动、连接、停止、重启多个窗格。
- **Session 延续**：为支持恢复的 Agent CLI 保留本机会话信息。
- **本地 Dashboard 与健康检查**：处理初始化、状态查看、配置编辑和常用操作。

## 安装

环境要求：通过 PyPI 或源码安装时需要 Python 3.10+，并安装配置中使用的 Agent CLI。`tmux` 是可选能力，只在使用 `layoutBackend: tmux` 的标签页时需要。
...
只有在你只想创建可复用的 tmux session、且不想进入工作台或打开 direct 布局外部进程时，才使用 `cc-branch start --detach`。
...
- `cc-branch plan`：启动前预览会创建哪些 session/pane。

...
from .remote import effective_remote, render_ssh_command
...
def _resolve_session_id(window: WindowConfig, state_entry: WindowState | None) -> tuple[str | None, str]:
    ...
    - omitted / ``auto``: use the bound state id when present
    ...
    if raw and raw != SESSION_AUTO:
        return raw, "explicit"
    return (state_entry.session_id if state_entry else None), SESSION_AUTO
...
def _build_window_plan(
    ...
    remote = effective_remote(slot.remote, window.remote)
    ...
    if remote is not None:
        launch_command = render_ssh_command(remote, launch_command)
        post_launch_commands = [
            render_ssh_command(remote, command)
            for command in post_launch_commands
        ]
        command_binary = "ssh"
    resume_mode = (
        "none"

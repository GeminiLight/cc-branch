...
class WorkspaceLifecycleActions:
    ...
    def restart_workspace(
        ...
        *,
        ...
        tmux_slots = [slot for slot in managed_slots(plan.slots) if slot.launchable_windows()]
        if not tmux_slots:
            return ActionResult(
                ok=False,
    ...
    def launch_workspace(
        ...
        *,
        ...
        slots = [slot for slot in slots if slot.launchable_windows()]
        if not slots:
            return ActionResult(
                ok=False,
                code="no_tmux_runtime",
                message="No enabled tmux runtime panes can be started in background. Open terminal slots directly.",
                exit_code=1,
            )

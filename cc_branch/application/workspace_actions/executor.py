...
class WorkspaceActionExecutor:
    ...
    def execute(
        ...
        *,
        ...
        if action == "launch":
            terminal_slots = [slot for slot in self.targets.terminal_slots(plan) if slot.launchable_windows()]
            tmux_slots = [slot for slot in self.targets.tmux_slots(plan) if slot.launchable_windows()]

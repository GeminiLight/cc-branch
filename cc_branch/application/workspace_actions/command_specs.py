...
def _launchable_windows(slot):
    if hasattr(slot, "launchable_windows"):
        return slot.launchable_windows()
    return [window for window in getattr(slot, "windows", []) if getattr(window, "enabled", True)]

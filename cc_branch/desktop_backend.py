...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--config", required=True, help=f"path to {DEFAULT_CONFIG}")
    parser.add_argument("--state", required=True, help=f"path to {DEFAULT_STATE}")
    parser.add_argument(
        "--token",
        default=None,
        help="optional bearer token for API access; desktop sidecars do not read CC_BRANCH_WEB_TOKEN by default",
    )
    return parser

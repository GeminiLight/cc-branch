...
@dataclass
class RemoteConfig:
    """SSH target used to run a tab, pane, or window on a remote machine."""
    ...
    @classmethod
    def from_dict(cls, data: Any) -> RemoteConfig | None:
        if data is None:
            return None
        if data is False:
        ...
        if isinstance(data, str):
            host = data.strip()
            return cls(host=host) if host else None
        ...
        return cls(
            ...
            user=str(data.get("user")).strip() if data.get("user") is not None else None,
            port=port,
            cwd=str(data.get("cwd")).strip() if data.get("cwd") is not None else None,
            args=[str(arg) for arg in args] if isinstance(args, list) else [],
            options=dict(data.get("options") or {}) if isinstance(data.get("options") or {}, dict) else {},
        )

    def to_dict(self) -> dict[str, Any] | bool:
        if not self.enabled:
            return False
        out: dict[str, Any] = {}
        if self.host:
...
class SlotConfig:
    ...
    Runtime/planner code calls the executable unit a slot, while the public
    config schema presents the top-level container as a tab.
    """
...
def _tabs_to_slots(raw_tabs: list[Any], default_layout_backend: str) -> list[SlotConfig]:
    ...
    for raw_tab in raw_tabs:
        ...
        tab_remote = RemoteConfig.from_dict(raw_tab.get("remote"))
        ...
        if tmux_panes:
            ...
            def append_direct_chunk(
                chunk: list[dict[str, Any]],
                fallback_index: int,
                *,
                tab_name: str = tab_name,
                tab_layout: str = tab_layout,
                tab_opener: Any = tab_opener,
                tab_cwd: str = tab_cwd,
                tab_env: dict[str, str] = tab_env,
                tab_remote: RemoteConfig | None = tab_remote,
                used_slot_names: set[str] = used_slot_names,
            ) -> None:
                ...
                for offset, pane in enumerate(chunk):
                    ...
                    slots.append(
                        SlotConfig(
                            ...
                            cwd=tab_cwd,
                            env=tab_env,
                            remote=RemoteConfig.from_dict(pane.get("remote")) or tab_remote,
            ...
            def append_tmux_pane(
                pane: dict[str, Any],
                index: int,
                *,
                tab_name: str = tab_name,
                tab_layout: str = tab_layout,
                tab_layout_backend: str = tab_layout_backend,
                tab_opener: Any = tab_opener,
                tab_cwd: str = tab_cwd,
                tab_env: dict[str, str] = tab_env,
                tab_remote: RemoteConfig | None = tab_remote,
                direct_panes: list[dict[str, Any]] = direct_panes,
                tmux_panes: list[dict[str, Any]] = tmux_panes,
                used_slot_names: set[str] = used_slot_names,
            ) -> None:
                nonlocal emitted_for_tab
...
def _slot_to_tab(slot: SlotConfig, default_layout_backend: str) -> dict[str, Any]:
    ...
    if slot.env:
        tab["env"] = slot.env
    if slot.remote is not None:
        tab["remote"] = slot.remote.to_dict()
...
def _tmux_group_pane(slot: SlotConfig) -> dict[str, Any]:
    ...
    if slot.env:
        pane["env"] = slot.env
    if slot.remote is not None:
        pane["remote"] = slot.remote.to_dict()
...
def _terminal_panes(slot: SlotConfig, default_layout_backend: str) -> list[dict[str, Any]]:
    ...
    if slot.remote is not None:
        for pane in panes:
            pane.setdefault("remote", slot.remote.to_dict())
    return panes
...
def _slot_public_windows(slot: SlotConfig) -> list[dict[str, Any]]:
    if slot.windows:
        ...
        for window in slot.windows:
            window_pane = _as_public_dict(window)
            if window.remote is not None:
                window_pane["remote"] = window.remote.to_dict()
            panes.append(window_pane)
        return panes
...
class WorkspaceConfig:
    ...
    def from_dict(cls, data: dict[str, Any]) -> WorkspaceConfig:
        ...
        open_with = _opener_id(data.get("openWith"))
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.agent_overrides:
            out["agents"] = {k: _as_public_dict(v) for k, v in self.agent_overrides.items()}
        if self.open_with:
            out["openWith"] = self.open_with

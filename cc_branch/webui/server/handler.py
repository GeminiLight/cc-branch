...
from .static import canonical_static_path, read_index_html, read_static_bytes
...
class WebUIHandler(BaseHTTPRequestHandler):
    ...
    def _route_post(self, path: str) -> None:
        if path == "/api/action":
            ...
            if self._require_auth():
                api.api_init(self)
        elif path in {"/api/config", "/api/window-enabled"}:
            if self._require_auth():
                (api.api_save_config if path == "/api/config" else api.api_window_enabled)(self)
        ...
        elif path == "/api/openers/global":
            if self._require_auth():
                api.api_save_global_openers(self)
        elif path == "/api/project/pick-directory" and self._require_auth():
            api.api_project_pick_directory(self)
        elif path == "/api/remote/list-directories" and self._require_auth():

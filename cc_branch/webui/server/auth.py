...
def is_trusted_desktop_origin(host: str | None) -> bool:
    """Return whether *host* is a packaged desktop app origin."""
    if not host:
        return False
    return host.strip().strip("[]").lower() == "tauri.localhost"
...
def origin_is_allowed(origin: str, *, host_header: str | None, token: str | None) -> bool:
    ...
    if parsed.scheme not in {"http", "https", "tauri", "asset"} or not parsed.hostname:
        return False

    host_name = urlparse(f"http://{host_header}").hostname if host_header else None
    if parsed.scheme in {"tauri", "asset"} and parsed.hostname in {"localhost", "tauri.localhost"}:
        return True
    if is_loopback_host(parsed.hostname) or is_trusted_desktop_origin(parsed.hostname):
        return True

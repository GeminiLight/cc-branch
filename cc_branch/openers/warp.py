...
class WarpLauncher:
    ...
    def open_layout(self, commands: list[OpenCommandSpec], *, name: str = "CC Branch") -> None:
        ...
        self.cleanup_stale_launch_configs(name, keep=path)
        self.cleanup_stale_launch_configs(launch_name, keep=path)
    ...
    def cleanup_stale_launch_configs(self, name: str, *, keep: Path) -> None:
        stale_pattern = f"{_slug(name)}-*.yaml"
        for path in keep.parent.glob(stale_pattern):
            if path == keep:
                continue
            try:
                path.unlink()

...
def ensure_terminal_slot(
    ...
    *,
    ...
    opener_id = slot.opener or open_with or "auto-terminal"
    results: list[AppliedWindowResult] = []
    for window in slot.launchable_windows():
...
def ensure_slot(
    ...
    *,
    ...
    launchable_windows = slot.launchable_windows()
    ...
    results = [
        execution._ensure_window(
            slot.tmux_session,
            slot,
            launchable_windows[0],
            is_first=True,
            created_action=created_action,
        )
    ]
    for window in launchable_windows[1:]:
        results.append(
            execution._ensure_window(
                slot.tmux_session,
                slot,

...
def apply_workspace(plan: WorkspacePlan, detach: bool = False) -> list[AppliedWindowResult]:
    ...
    background_slots = [
        ...
        if supports_background_start(slot.runtime) and slot.launchable_windows()
    ]
    ...
    first_slot = next(
        (slot for slot in plan.slots if supports_attach(slot.runtime) and slot.launchable_windows()),
        None,
    )
...
def attach_slot(plan: WorkspacePlan, slot_name: str) -> None:
    ...
    if is_external_process_runtime(slot.runtime):
        execution.ensure_slot(slot, custom_openers=plan.openers, open_with=plan.open_with)
        return
...
def restart_workspace(
    ...
    if is_external_process_runtime(slot.runtime):
        execution.open_command(
            opener_id=window.opener or slot.opener or plan.open_with or "auto-terminal",
            cwd=Path(window.cwd),

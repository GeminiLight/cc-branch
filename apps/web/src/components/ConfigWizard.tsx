...
function templateRuntimeLabelKey(stats: ReturnType<typeof templateStats>): string {
  if (stats.tmuxTabs > 0 && stats.directTabs > 0) return "templateMixedRuntime";
  if (stats.tmuxTabs > 0) return "templateTmuxRuntime";
  ...
  return (
    <div className="space-y-3">
      {spec.tabs.map((tab, tabIndex) => {
        const panes = tab.layoutBackend === "direct" && tab.panes.length === 0
          ? [{ name: tab.name, preferredAgents: ["shell"], agent: "shell" }]
          : tab.panes;
        const isSingleShellDirectTab = tab.layoutBackend === "direct" && tab.panes.length === 0;
        return (
...
function TemplateCardMap({ spec }: { spec: TemplateSpec }) {
  const { t } = useI18n();
  return (
    <div className="mt-2 space-y-1.5" aria-label={t("workspaceShape")}>
      {spec.tabs.map((tab, tabIndex) => (
        <div key={tabIndex} className="rounded-md border border-subtle bg-[var(--bg-elevated)] px-2 py-1.5">
          <div className="flex items-center gap-1.5 text-[10px] font-semibold text-primary">
            {tab.layoutBackend === "tmux" ? (
              <Layers3 className="h-3 w-3 shrink-0 text-[var(--accent)]" />
            ) : (
              <TerminalSquare className="h-3 w-3 shrink-0 text-[var(--accent)]" />
            )}
            <span className="uppercase text-tertiary">{t("tab")}</span>
            <span className="truncate font-mono">{tab.name}</span>
          </div>
          <div className="mt-1 grid gap-1">
            {tab.panes.slice(0, 3).map((pane, paneIndex) => (
              <div key={paneIndex} className="grid grid-cols-[38px_minmax(0,1fr)] items-center gap-1 rounded border border-default bg-[var(--bg-card)] px-1.5 py-0.5">
                <span className="text-[9px] font-semibold uppercase text-tertiary">{t("pane")}</span>
                <span className="truncate font-mono text-[10px] text-secondary">{pane.name}</span>
              </div>
            ))}
            {tab.panes.length > 3 && (
              <div className="rounded border border-dashed border-default px-1.5 py-0.5 text-[10px] text-tertiary">
                +{tab.panes.length - 3}
              </div>
            )}
          </div>
...
export default function ConfigWizard({ projectPath, configPath, isOpen, onClose, onCreated }: ConfigWizardProps) {
  ...
  const [customTemplates, setCustomTemplates] = useState<TemplateSpec[]>(() => isOpen ? loadUserTemplates() : []);
  const [draftSpec, setDraftSpec] = useState<TemplateSpec>(() => {
    const specs = effectiveTemplateSpecs(isOpen ? loadUserTemplates() : []);
    return cloneTemplate(specs[defaultProfileId] || templateSpecs[defaultProfileId]);
  });
  ...
  useEffect(() => {
    if (!isOpen) return;
    setCustomTemplates(loadUserTemplates());
  }, [isOpen]);

  const allTemplateSpecs = useMemo(() => effectiveTemplateSpecs(customTemplates), [customTemplates]);
  const customProfiles = useMemo(() => customProfileTemplates(customTemplates), [customTemplates]);

  const visibleProfiles = useMemo(() => {
    const byId = new Map((profiles?.length ? profiles : fallbackProfiles).map((profile) => [profile.id, profile]));
    return [
      ...profileOrder.map((id) => byId.get(id) || { id, description: "" }),
      ...customProfiles.map((template) => ({ id: template.id, description: template.name || template.id })),
    ];
  ...
  useEffect(() => {
    ...
    if (!visibleProfiles.some((profile) => profile.id === selectedProfileId)) {
      ...
      setDraftSpec(cloneTemplate(allTemplateSpecs[nextId] || templateSpecs[defaultProfileId]));
    }
  }, [allTemplateSpecs, isOpen, selectedProfileId, visibleProfiles]);

  useEffect(() => {
    if (!isOpen) return;
    setDraftSpec(cloneTemplate(allTemplateSpecs[selectedProfileId] || templateSpecs[defaultProfileId]));
  }, [allTemplateSpecs, isOpen, selectedProfileId]);
  ...
  const selectProfile = useCallback((profileId: string) => {
    ...
    setDraftSpec(cloneTemplate(allTemplateSpecs[profileId] || templateSpecs[defaultProfileId]));
  }, [allTemplateSpecs]);
          ...
          {step === "select" && (
            <div className="grid min-h-full min-w-0 lg:grid-cols-[280px_minmax(0,1fr)]">
              <div className="min-w-0 overflow-hidden border-b lg:border-b-0 lg:border-r border-default bg-[var(--bg-elevated)]/45 p-3 sm:p-4">
                ...
                <div className="mt-3 grid gap-2">
                  {visibleProfiles.map((profile) => {
                    ...
                    const spec = allTemplateSpecs[profile.id];
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-[13px] font-semibold text-primary">
                            {profileLabelKeys[profile.id] ? t(profileLabelKeys[profile.id]) : spec?.name || profile.description || profile.id}
                          </p>
                          {spec && (
                            <span className="mt-2 flex flex-wrap items-center gap-1.5">
                              ...
                              <span className="rounded border border-default bg-[var(--bg-elevated)] px-1.5 py-0.5 text-[10px] font-semibold text-tertiary">
                                {stats ? t(templateRuntimeLabelKey(stats)) : t("terminalRuntime")}
                              </span>
                            </span>
                          )}
                          {spec && <TemplateCardMap spec={spec} />}
                        </div>
              ...
              <div className="flex min-h-full min-w-0 flex-col p-3 sm:p-4">
                ...
                <div className="mt-3 flex flex-col sm:flex-row sm:items-center justify-end gap-2 rounded-md border border-default bg-[var(--bg-card)] px-3 py-2.5">

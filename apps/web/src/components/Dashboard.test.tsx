...
vi.mock('../hooks', () => ({
  ...
  useWindowEnabled: () => ({
    mutateAsync: mocks.windowEnabledMutateAsync,
    isPending: false,
  }),
  useApiClient: () => ({
    getConfig: vi.fn(),
...
describe('Dashboard actions', () => {
  ...
  it('defaults the workspace launch tool to VS Code when it is available', async () => {
    renderDashboard()
  ...
  it('localizes built-in opener labels without changing the opener id', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '在 VS Code 启动工作空间' }))
  ...
  it('lists editor tools and opens them as workspaces', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Tool: VS Code' }))
  ...
  it('shows app logos in the workspace tool selector', () => {
    mocks.openersResult.current = {
      data: {
        default: 'auto-terminal',
        openers: [
    ...
    expect(screen.getByLabelText('VS Code logo')).toBeInTheDocument()

    fireEvent.click(screen.getByRole('button', { name: 'Tool: VS Code' }))

    expect(within(screen.getByRole('option', { name: 'System Terminal' })).getByLabelText('System Terminal logo')).toBeInTheDocument()
  ...
  it('keeps background lifecycle actions out of the header', () => {
    ...
    expect(screen.getByRole('button', { name: 'Launch workspace in VS Code' })).toBeInTheDocument()
  ...
  it('opens the setup wizard with profile, agent, and target context', () => {
    ...
    expect(within(dialog).queryByText('tmux + terminal')).not.toBeInTheDocument()

...
import { AlertTriangle, CheckCircle2, ClipboardList, RefreshCw, Stethoscope, XCircle } from "lucide-react";
import { useI18n } from "../i18n";
import { useApiClient, useConfig, useDoctor, useWorkspace, useWorkspaceAction } from "../hooks";
...
export default function DoctorView({ projectPath, configPath }: DoctorViewProps) {
  ...
  const diagnosticReport = useMemo(() => {
    const lines = [
      ...
      "",
      "findings:",
      ...actionableChecks.map((check) => `- ${check.status}: ${check.text}${check.fix ? ` | fix: ${check.fix}` : ""}`),
      ...(actionableChecks.length === 0 ? ["- none"] : []),
      "",
      "passing:",
      ...passingChecks.map((check) => `- ${check.text}`),
    ];
    return lines.join("\n");
  }, [actionableChecks, configPath, issueCount, overallLabel, passedCount, passingChecks, projectPath, warningCount]);
  ...
  const copyDiagnosticReport = async () => {
    try {
      const bundle = await api.getDiagnosticBundle(scope);
        ...
        actions={
          <div className="flex flex-wrap items-center justify-end gap-2">
            <button
              ...
              className="control-touch px-3 rounded-md text-[12px] font-medium text-secondary hover:text-primary surface-card border border-default hover:border-[var(--border-strong)] transition-colors flex items-center justify-center gap-1.5"
            >

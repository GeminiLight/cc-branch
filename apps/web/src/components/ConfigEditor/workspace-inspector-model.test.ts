...
describe("workspace inspector model", () => {
  ...
  it("moves tmux-backed tabs as tabs and explicit tmux groups as panes", () => {
    const tmuxBackedTabs = [
    ...
    const tabBacked = deriveWorkspaceSelection(tmuxBackedTabs, { slotIndex: 1, target: "pane", windowIndex: null });
    expect(selectedTmuxGroupPositionState(tmuxBackedTabs, tabBacked)).toEqual({
      isTabBacked: true,
      canMoveUp: true,
      canMoveDown: false,
    });

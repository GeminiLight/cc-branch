...
export function isTmuxBackedTab(slot: SlotConfig | null | undefined): boolean {
...
export function configuredPaneCount(slot: SlotConfig): number {
  if (isTmuxBackedTab(slot)) return 1;
...
export function addTmuxGroupPaneMutation(
  ...
  const slot = slots[slotIndex];
  if (!slot) return null;
  const panes = isTmuxBackedTab(slot) ? [tmuxGroupWindowFromSlot(slot)] : editableWindowsForSlot(slot);
...
export function tmuxGroupWindows(window: WindowConfig | null | undefined): WindowConfig[] {
  ...
  return [{
    ...
    enabled: window.enabled === false ? false : undefined,
    cwd: window.cwd ?? null,
    env: { ...window.env },
...
export function slotToCanvasPanes(slot: SlotConfig): CanvasPane[] {
  ...
  return panes.map((pane, index) => ({
    ...
    ...(pane.enabled === false ? { enabled: false } : {}),
...
export function canDragPane(slot: SlotConfig): boolean {
  return isTmuxBackedTab(slot) || slot.runtime === "terminal" || slot.windows.length > 0;
}
...
export function movePaneBetweenSlots(
  ...
  if (fromSlotIndex === toSlotIndex && isTmuxBackedTab(source)) return null;
  ...
  const sourceIsTmuxBackedTabGroup = isTmuxBackedTab(source);
  if (sourceIsTmuxBackedTabGroup && fromPaneIndex !== 0) return null;
  const sourceWindows = sourceIsTmuxBackedTabGroup ? [] : editableWindowsForSlot(source);
  if (!sourceIsTmuxBackedTabGroup && fromPaneIndex >= sourceWindows.length) return null;

  const moved = sourceIsTmuxBackedTabGroup
    ? tmuxGroupWindowFromSlot(source)
    : sourceWindows.splice(fromPaneIndex, 1)[0];
  if (!moved) return null;

  const targetWindows = isTmuxBackedTab(target) ? [tmuxGroupWindowFromSlot(target)] : editableWindowsForSlot(target);
  const insertIndex = Math.min(Math.max(toPaneIndex, 0), targetWindows.length);
  targetWindows.splice(insertIndex, 0, moved);

  const next = [...slots];
  if (sourceIsTmuxBackedTabGroup || sourceWindows.length === 0) {

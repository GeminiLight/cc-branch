...
import type { RemoteSetting, WindowConfig } from "./types";
import {
  FieldLabel,
  KeyValueList,
  SelectInput,
...
import { DefaultShellCommandHint, LaunchToggle } from "./WorkspaceDetailEditors";
...
export default function TmuxGroupEditor({
  ...
  groupRemote?: RemoteSetting;
  groupInheritedRemote?: RemoteSetting;
  inheritedRemote?: RemoteSetting;
  onGroupRemoteChange?: (remote: RemoteSetting) => void;
  ...
  return (
    <div className="space-y-3">
      ...
      {onGroupRemoteChange && (
        <RemoteRunEditor
          value={groupRemote}
          inheritedRemote={groupInheritedRemote}
          sshHosts={sshHosts}
          onChange={onGroupRemoteChange}
        />
      )}
        ...
        <div className="mt-2 space-y-2">
          {windows.map((window, windowIndex) => (
            <div
              ...
              className={`rounded-md border border-default bg-[var(--bg-card)] p-2 shadow-sm ${window.enabled === false ? "opacity-70" : ""}`}
            >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="truncate text-[10px] text-tertiary">
                      {window.enabled === false
                        ? t("disabled")
                        : window.agent
                        ? displayAgentName(window.agent)
                        : window.command || defaultShellDisplay}
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 gap-2">
                <LaunchToggle
                  ...
                  onChange={(enabled) => onUpdateWindow(windowIndex, { enabled })}
                />
                <div>
                ...
                {window.agent ? (
                  <div>
                    ...
                    <SessionInput
                      ...
                      onChange={(value) => onUpdateWindow(windowIndex, { session: value || null })}
                      agent={window.agent}
                      scope={scope}
                    />
                  </div>
              ...
              <details className="group mt-2 rounded-md border border-subtle bg-[var(--bg-hover)]/25">
                ...
                <div className="px-2.5 pb-2.5 pt-0 space-y-2.5">
                  ...
                  <RemoteRunEditor
                    ...
                    sshHosts={sshHosts}
                    onChange={(remote) => onUpdateWindow(windowIndex, { remote })}
                    compact
                  />
                  <div>

 ...
 *
 * Workspace canvas for editing public v2 tabs/panes.
 */
...
export default function SlotsSection({
  ...
  function togglePaneEnabled(slotIndex: number, windowIndex: number | null) {
    const slot = slots[slotIndex];
    if (!slot) return;
    const next = [...slots];
    if (slot.windows.length === 0) {
      const window = terminalSlotToWindow(slot);
      next[slotIndex] = {
        ...slot,
        command: undefined,
        title: undefined,
        ...
        session: undefined,
        label: undefined,
        windows: [{ ...window, enabled: window.enabled === false ? undefined : false }],
      };
      onChange(next);
      return;
    }
    const index = windowIndex ?? 0;
    const window = slot.windows[index];
    if (!window) return;
    const windows = [...slot.windows];
    windows[index] = { ...window, enabled: window.enabled === false ? undefined : false };
    next[slotIndex] = { ...slot, windows };
    onChange(next);
  }
  ...
  const selectedTmuxWindowList = selectedTmuxGroup
    ? selectedSlot && isTmuxBackedTab(selectedSlot)
      ? slotToPanes(selectedSlot)
      : selectedWindow
      ? tmuxGroupWindows(selectedWindow)
      : []
    : [];
  const selectedTmuxGroupRemote = selectedSlot && isTmuxBackedTab(selectedSlot) ? selectedSlot.remote : selectedWindow?.remote;
  const selectedTmuxGroupInheritedRemote = selectedSlot && isTmuxBackedTab(selectedSlot) ? undefined : selectedSlot?.remote;
  const selectedTmuxWindowInheritedRemote = selectedTmuxGroupRemote === false
    ? null
    : selectedTmuxGroupRemote ?? selectedTmuxGroupInheritedRemote;

  function updateSelectedTmuxGroupName(value: string) {
    if (!selectedSlot) return;
    if (isTmuxBackedTab(selectedSlot)) {
  ...
  const {
    isTabBacked: selectedTmuxGroupIsTabBacked,
  ...
  function moveSelectedTmuxGroup(dir: number) {
    if (selectedTmuxGroupIsTabBacked) {
  ...
  const canvasProps = {
    ...
    onTogglePaneEnabled: togglePaneEnabled,
                      ...
                      {selectedSlot.runtime === "terminal" && !selectedTmuxGroup ? (
                        <TerminalPaneEditor
                          ...
                          onLaunchEnabledChange={(enabled) => {
                            if (selectedTerminalWindow) updateWindow(normalizedSelection.windowIndex ?? 0, { enabled });
                            else togglePaneEnabled(normalizedSelection.slotIndex, null);
                          }}
                        />
                      ) : selectedTmuxGroup ? (
                        <TmuxGroupEditor
                          groupName={isTmuxBackedTab(selectedSlot) ? selectedSlot.name : selectedWindow?.name ?? ""}
                          windows={selectedTmuxWindowList}
                          agentOptions={agentOptions}
                          scope={scope}
                          defaultShellName={defaultShellName}
                          sshHosts={sshHosts}
                          groupRemote={selectedTmuxGroupRemote}
                          groupInheritedRemote={selectedTmuxGroupInheritedRemote}
                          inheritedRemote={selectedTmuxWindowInheritedRemote}
                          onGroupRemoteChange={(remote) => {
                            if (!selectedSlot) return;
                            if (isTmuxBackedTab(selectedSlot)) updateSlot(normalizedSelection.slotIndex, { remote });
                            else updateWindow(normalizedSelection.windowIndex ?? 0, { remote });
                          }}

...
export function deriveWorkspaceSelection(slots: SlotConfig[], selection: Selection): WorkspaceSelectionState {
  ...
  const selectedWindow =
    normalizedSelection.target === "pane" && !isTmuxBackedTab(selectedSlot) && selectedSlot?.windows.length
  ...
  const selectedTmuxGroup =
    normalizedSelection.target === "pane" &&
    (isTmuxBackedTab(selectedSlot) || isTmuxGroupWindow(selectedWindow));
...
function paneSelectionForSlot(
  ...
  if (isTmuxBackedTab(slot) || slot.windows.length === 0) {

...
import { isTmuxBackedTab } from "./workspace-model";
...
export function inspectorSelectionSubtitle(
  ...
  if (state.selectedTmuxGroup) {
    if (isTmuxBackedTab(slot)) return slot.name || t("unnamed");
...
export function selectedTmuxGroupPositionState(
  ...
  const isTabBackedGroup = Boolean(state.selectedTmuxGroup && isTmuxBackedTab(state.selectedSlot));
  const windowIndex = state.normalizedSelection.windowIndex ?? 0;
  return {
    isTabBacked: isTabBackedGroup,
    canMoveUp: isTabBackedGroup
      ? state.normalizedSelection.slotIndex > 0
      : Boolean(state.selectedTmuxGroup && windowIndex > 0),
    canMoveDown: isTabBackedGroup

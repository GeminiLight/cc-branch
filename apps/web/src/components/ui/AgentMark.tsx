...
export function normalizeAgentKey(agent: string | null | undefined): string {
  ...
  if (compact.includes("opencode")) return "opencode";
  if (compact.includes("copilot") || compact.includes("githubcopilot")) return "copilot";
  return value;
}

function initialsForAgent(label: string): string {
  const parts = label.split(/[\s_-]+/).filter(Boolean);
  if (parts.length >= 2) return `${parts[0][0] || ""}${parts[1][0] || ""}`.toUpperCase();
  return label.slice(0, 2) || "A";
}
...
function agentIdentity(agent: string | null | undefined) {
  ...
  if (key === "opencode") return { label: "OpenCode", initials: "OC", iconUrl: opencodeIconUrl, tone: "bg-[#1b1717] text-white border-[#312c2c]" };
  if (key === "copilot") return { label: "GitHub Copilot", initials: "GH", iconUrl: copilotIconUrl, tone: "bg-[#f6f8fa] text-[#24292f] border-[#d0d7de]" };
  if (agent) {
    const label = displayAgentName(agent);
    return {
      label,
      initials: initialsForAgent(label),
      tone: "bg-[#f8fafc] text-[#334155] border-[#cbd5e1] dark:bg-[#172033] dark:text-[#cbd5e1] dark:border-[#475569]",
    };
  }

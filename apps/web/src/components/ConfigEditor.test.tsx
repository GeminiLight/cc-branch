...
vi.mock('../hooks', () => ({
  useApiInfo: () => ({
    data: {
      ...
      default_shell: 'zsh',
      ssh_hosts: [
        { alias: 'gpu-dev', hostname: 'gpu.example.com', user: 'ubuntu', port: 2222 },
        { alias: 'staging' },
      ],
    },
  }),
...
describe('ConfigEditor diagnostics', () => {
  ...
  it('shows app logos in the default launch tool selector', () => {
    renderConfigEditor('project')

    fireEvent.click(screen.getByRole('button', { name: /None/ }))

    expect(within(screen.getByRole('option', { name: 'Cursor' })).getByLabelText('Cursor logo')).toBeInTheDocument()
    expect(within(screen.getByRole('option', { name: 'VS Code' })).getByLabelText('VS Code logo')).toBeInTheDocument()
    expect(within(screen.getByRole('option', { name: 'Warp' })).getByLabelText('Warp logo')).toBeInTheDocument()
  })

  it('lets users choose an SSH run location from the form editor', async () => {
    const currentResult = mocks.configResult.current as { data: Record<string, unknown> }
    mocks.configResult.current = {
      ...currentResult,
      data: {
    ...
    fireEvent.change(screen.getByPlaceholderText('devbox'), { target: { value: 'gpu-dev' } })
    fireEvent.change(screen.getByPlaceholderText('/srv/app'), { target: { value: '/srv/app' } })
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
    ...
    expect(content).toContain('host: gpu-dev')
    expect(content).toContain('cwd: /srv/app')
  })

  it('lets users choose a saved SSH target from local SSH config', async () => {
    const currentResult = mocks.configResult.current as { data: Record<string, unknown> }
    mocks.configResult.current = {
      ...currentResult,
      data: {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'SSH' }))
    fireEvent.click(screen.getByRole('button', { name: 'SSH target' }))
    fireEvent.click(await screen.findByRole('option', { name: 'gpu-dev (ubuntu@gpu.example.com:2222)' }))
    fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  ...
  it('moves a tmux group from the inspector', async () => {
    const currentResult = mocks.configResult.current as { data: Record<string, unknown> }
    mocks.configResult.current = {
      ...currentResult,
      data: {
  ...
  it('reorders a leading pane before a later pane by dragging on the workspace matrix', async () => {
    const currentResult = mocks.configResult.current as { data: Record<string, unknown> }
    mocks.configResult.current = {
      ...currentResult,
      data: {
    ...
    const dataTransfer = createDataTransfer()
    fireEvent.dragStart(uiPane, { dataTransfer })
    expect(dataTransfer.setData).toHaveBeenCalledWith('text/plain', '0:0')
    fireEvent.dragOver(docsPane, { dataTransfer, clientX: 20, clientY: 20 })
    fireEvent.drop(docsPane, { dataTransfer, clientX: 20, clientY: 20 })
  ...
  it('reorders tabs by dragging rows on the workspace matrix', () => {
    ...
    const devTab = screen.getByRole('button', { name: 'Edit tab dev' })
    const devRow = devTab.closest('section')!
    ...
    const dataTransfer = createDataTransfer()
    fireEvent.dragStart(devTab, { dataTransfer })

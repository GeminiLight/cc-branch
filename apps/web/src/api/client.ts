...
import type {
  ...
  WindowEnabledRequest,
  OpenersData,
  GlobalOpenersData,
  GlobalOpenersSaveResult,
  AgentsData,
  AgentSessionsData,
  ConfigSaveResult,
  ConfigIssue,
...
export interface APIClient {
  shouldInjectCurrentProject(): boolean;
  ...
  getDiagnosticBundle(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<DiagnosticBundle>;
  ...
  listRemoteDirectories(remote: Omit<RemoteProjectInput, "cwd">, path?: string): Promise<RemoteDirectoryListing>;
  revealPath(path: string): Promise<ActionResult>;
  revealDesktopApp(): Promise<ActionResult | null>;
  getOpeners(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<OpenersData>;
  getGlobalOpeners(signal?: AbortSignal): Promise<GlobalOpenersData>;
  saveGlobalOpeners(content: string, baseMtime?: number | null, baseContentHash?: string | null): Promise<GlobalOpenersSaveResult>;
  ...
  restartBackend(): Promise<ApiInfo | null>;
  openExternalUrl(url: string): Promise<void>;
  getProjectsIndex(signal?: AbortSignal): Promise<ProjectsIndexData>;
  addProject(request: string | AddProjectRequest, name?: string): Promise<ProjectsIndexData>;
...
const DEFAULT_BACKEND_CONNECTION_ERROR =
...
type AppScopeWindow = Window & { __CC_BRANCH_APP_SCOPE__?: boolean };

async function fetchApi(input: RequestInfo | URL, init?: RequestInit, failureMessage = DEFAULT_BACKEND_CONNECTION_ERROR): Promise<Response> {
  try {
...
export class HTTPClient implements APIClient {
  ...
  shouldInjectCurrentProject(): boolean {
    return typeof window === "undefined" || !(window as AppScopeWindow).__CC_BRANCH_APP_SCOPE__;
  }
  ...
  async getDiagnosticBundle(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<DiagnosticBundle> {
    const res = await fetchApi(`${this.baseUrl}/api/diagnostics/bundle${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as DiagnosticBundle;
  }
  ...
  async listRemoteDirectories(remote: Omit<RemoteProjectInput, "cwd">, path?: string): Promise<RemoteDirectoryListing> {
    const res = await fetchApi(`${this.baseUrl}/api/remote/list-directories`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(typeof data.error === "string" ? data.error : `HTTP ${res.status}`);
    return data as RemoteDirectoryListing;
  }

  async revealPath(path: string): Promise<ActionResult> {
    const res = await fetchApi(`${this.baseUrl}/api/system/reveal`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async revealDesktopApp(): Promise<ActionResult | null> {
    return null;
  }
  ...
  async getGlobalOpeners(signal?: AbortSignal): Promise<GlobalOpenersData> {
    const res = await fetchApi(`${this.baseUrl}/api/openers/global`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as GlobalOpenersData;
  }

  async saveGlobalOpeners(content: string, baseMtime?: number | null, baseContentHash?: string | null): Promise<GlobalOpenersSaveResult> {
    const res = await fetchApi(`${this.baseUrl}/api/openers/global`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        content,
    ...
    return data as GlobalOpenersSaveResult;
  }
  ...
  async setWindowEnabled(request: WindowEnabledRequest): Promise<ActionResult> {
    const { projectPath, configPath, ...body } = request;
    const res = await fetchApi(`${this.baseUrl}/api/window-enabled${qs({ projectPath, configPath })}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async restartBackend(): Promise<ApiInfo | null> {
    return null;
  }
  ...
  async addProject(request: string | AddProjectRequest, name?: string): Promise<ProjectsIndexData> {
    const body = typeof request === "string" ? { path: request, name } : request;
...
export class TauriClient implements APIClient {
  private _lastApiInfo: ApiInfo | null = null;
  ...
  private _backendStartupError(info: ApiInfo): Error {
    const lines = [
      "CC Branch desktop backend did not start.",
      `Desktop version: ${info.desktop_version || "unknown"}`,
      `Desktop platform: ${info.desktop_platform || "unknown"}/${info.desktop_arch || "unknown"}`,
      `Backend source: ${info.backend_source || "unknown"}`,
      `Port: ${Number.isFinite(info.port) ? info.port : "unknown"}`,
      `Config: ${info.config_path || "unknown"}`,
      `State: ${info.state_path || "unknown"}`,
    ];
    if (info.startup_error) {
      lines.push(`Error: ${info.startup_error}`);
    }
  ...
  private _backendConnectionError(): string {
    ...
    const lines = [
      ...
      `Desktop platform: ${info?.desktop_platform || "unknown"}/${info?.desktop_arch || "unknown"}`,
      `Backend source: ${info?.backend_source || "unknown"}`,
      `Port: ${info && Number.isFinite(info.port) ? info.port : "unknown"}`,
      `Config: ${info?.config_path || "unknown"}`,
      `State: ${info?.state_path || "unknown"}`,
  ...
  private async _fetchApi(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
    return fetchApi(input, init, this._backendConnectionError());
  }

  private _assertBackendReady(info: ApiInfo): ApiInfo {
    if (info.backend_ready === false || !Number.isFinite(info.port) || info.port <= 0) {
      throw this._backendStartupError(info);
    }
    return info;
  }
  ...
  private async _baseUrl(signal?: AbortSignal): Promise<string> {
    ...
    const readyInfo = this._assertBackendReady(info);
    this._lastApiInfo = readyInfo;
    return `http://127.0.0.1:${readyInfo.port}`;
  }
  ...
  async getStatus(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<WorkspaceStatus> {
    ...
    const res = await this._fetchApi(`${baseUrl}/api/status${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return normalizeWorkspaceStatus(data);
  }
  ...
  async getConfig(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<ConfigData> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/config${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as ConfigData;
  }
  ...
  async getConfigs(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<ConfigOptionsData> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/configs${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as ConfigOptionsData;
  }
  ...
  async getDoctor(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<DoctorReport> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/doctor${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as DoctorReport;
  }
  ...
  async getDiagnosticBundle(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<DiagnosticBundle> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/diagnostics/bundle${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as DiagnosticBundle;
  }
  ...
  async probeProject(projectPath: string, signal?: AbortSignal): Promise<ProjectProbe> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/project/probe${qs(projectPath)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as ProjectProbe;
  }
  ...
  async listRemoteDirectories(remote: Omit<RemoteProjectInput, "cwd">, path?: string): Promise<RemoteDirectoryListing> {
    ...
    const res = await this._fetchApi(`${baseUrl}/api/remote/list-directories`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ remote, path }),
    });
  ...
  async revealPath(path: string): Promise<ActionResult> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/system/reveal`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ path }),
    });
  ...
  async revealDesktopApp(): Promise<ActionResult | null> {
    return this._invoke("reveal_desktop_app") as Promise<ActionResult>;
  }

  async getOpeners(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<OpenersData> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/openers${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as OpenersData;
  }
  ...
  async getGlobalOpeners(signal?: AbortSignal): Promise<GlobalOpenersData> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/openers/global`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as GlobalOpenersData;
  }
  ...
  async saveGlobalOpeners(content: string, baseMtime?: number | null, baseContentHash?: string | null): Promise<GlobalOpenersSaveResult> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/openers/global`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        content,
  ...
  async getAgents(scope?: WorkspaceScope | string, signal?: AbortSignal): Promise<AgentsData> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/agents${qs(scope)}`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as AgentsData;
  }
  ...
  async getGlobalAgents(signal?: AbortSignal): Promise<GlobalAgentsData> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/agents/global`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as GlobalAgentsData;
  }
  ...
  async saveGlobalAgents(content: string, baseMtime?: number | null, baseContentHash?: string | null): Promise<GlobalAgentsSaveResult> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/agents/global`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        content,
  ...
  async runWorkspaceAction(request: WorkspaceActionRequest): Promise<ActionResult> {
    const { projectPath, configPath, stopRemoved, ...body } = request;
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/action${qs({ projectPath, configPath })}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...body, stop_removed: stopRemoved }),
    });
  ...
  async setWindowEnabled(request: WindowEnabledRequest): Promise<ActionResult> {
    const { projectPath, configPath, ...body } = request;
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/window-enabled${qs({ projectPath, configPath })}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
  ...
  async restartBackend(): Promise<ApiInfo> {
    const info = await this._invoke("restart_backend") as ApiInfo;
    return this._assertBackendReady(info);
  }

  async openExternalUrl(url: string): Promise<void> {
    await this._invoke("open_external_url", { url });
  }

  async getProjectsIndex(signal?: AbortSignal): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/projects`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data as ProjectsIndexData;
  }
  ...
  async addProject(request: string | AddProjectRequest, name?: string): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl();
    const body = typeof request === "string" ? { path: request, name } : request;
    const res = await this._fetchApi(`${baseUrl}/api/projects/add`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
  ...
  async removeProject(id: string): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/projects/remove`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id }),
    });
  ...
  async activateProject(id: string): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/projects/activate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id }),
    });
  ...
  async setProjectPinned(id: string, pinned: boolean): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/projects/pin`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, pinned }),
    });
  ...
  async reorderProject(id: string, beforeId?: string | null): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/projects/reorder`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ id, before_id: beforeId ?? null }),
    });
  ...
  async injectCurrentProject(scope?: WorkspaceScope | string): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/projects/current${qs(scope)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({}),
    });
  ...
  async setProjectConfig(projectPath: string, configPath: string): Promise<ProjectsIndexData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/projects/config`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ project_path: projectPath, config_path: configPath }),
    });
  ...
  async createWorkspaceConfig(projectPath: string, name: string, sourceConfigPath?: string): Promise<ConfigOptionsData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/configs/create${qs(projectPath)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, source_config_path: sourceConfigPath }),
    });
  ...
  async renameWorkspaceConfig(projectPath: string, configPath: string, name: string): Promise<ConfigOptionsData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/configs/rename${qs(projectPath)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ config_path: configPath, name }),
    });
  ...
  async deleteWorkspaceConfig(projectPath: string, configPath: string): Promise<ConfigOptionsData> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/configs/delete${qs(projectPath)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ config_path: configPath }),
    });
  ...
  async getProfiles(signal?: AbortSignal): Promise<Profile[]> {
    const baseUrl = await this._baseUrl(signal);
    const res = await this._fetchApi(`${baseUrl}/api/profiles`, { signal });
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
    return data.profiles as Profile[];
  }
  ...
  async initWorkspace(profile: string, bootstrapSessions: boolean, scope?: WorkspaceScope | string): Promise<InitResult> {
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/init${qs(scope)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ profile, bootstrap_sessions: bootstrapSessions }),
    });
  ...
  async saveConfig(
    ...
    const baseUrl = await this._baseUrl();
    const res = await this._fetchApi(`${baseUrl}/api/config${qs(scope)}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        content,
...
export function createClient(): APIClient {
  ...
  const packagedDesktopProtocol = window.location.protocol !== "http:" && window.location.protocol !== "https:";
  if (
    isTauri() ||
    packagedDesktopProtocol ||
